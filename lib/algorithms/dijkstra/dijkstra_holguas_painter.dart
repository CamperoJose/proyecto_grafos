...
class Union extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var nodo in vUniones) {
      ...
      //verificar que el nodo inicial y nodo final se encuentran en la lista auxDijkstra:
      bool presente = false;
      for (var i = 0; i < auxDijkstra.length; i++) {
        if (auxDijkstra[i][0] == mensajeFinal && auxDijkstra[i][1] == mensajeInicial) {
          presente = true;
          break;
        }
        if (auxDijkstra[i][0] == mensajeInicial && auxDijkstra[i][1] == mensajeFinal) {
          presente = true;
          break;
        }
      }
